...
public class ItemService {
    ...
    public Money getTotalMinimumPricesOfBrandItems(String brandName) {
        ...
        if (prices.size() == 0) {
            throw new NotFoundItemException("해당 브랜드에 상품이 존재하지 않습니다.");
        }
    ...
    public MinAndMaxBrandItemDto getMinimumAndMaximumPriceByCategory(String categoryName) {
        ...
        if (prices.size() == 0) {
            throw new NotFoundCategoryException("해당 카테고리에 존재하는 상품을 찾을 수 없습니다.");
        }
    ...
    public CategoryBrandItemsDto getAllMinimumPricesByCategoryAndBrand(List<CategoryBrandDto> categoryBrandsDto) {
        ...
        return new CategoryBrandItemsDto(result);
    }
